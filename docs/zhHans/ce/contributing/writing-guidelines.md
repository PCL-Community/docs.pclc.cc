...
3. 请勿在图中添加影响阅读性的大型水印，您可以在图片右下/左下等位置添加水印以防止他人使用该图片。

4. 请勿使用 img1、1 等名字，请使用拼音或英文在图片名称中简要描述图片的用途。
...
5. 使用常见的图片格式，如 png、jpg、webp 等。

6. 如图片需要详细注释，可以在图片链接末尾以参数 "?comment=注释" 为格式添加。

7. 如一张图片中涉及多个操作，请使用矩形框出各步骤并用箭头顺次连接。

8. 如图片需要对应一段文字，请使用以下示例方案：
...
1. 添加图床为公开图床且符合上述要求，如该图床为私人自建图床，则应获取该图床管理者的许可。

2. 如您欲将您自建的图床添加至上述列表，请您确保您有足够的资金以应对 PCL 帮助内每日高额请求且您已做好了防范 DDoS 等恶意攻击的措施。
...
> **提示**: 该帮助页面还需要根据规范重新整理格式，优化排版与文案。如果你感兴趣也可以来帮帮忙！
